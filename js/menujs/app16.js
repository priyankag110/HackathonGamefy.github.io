(function($){
    ...
    function loopGallery(test, index, item){
        if(test){
            ...
            var mask = $('<div class="mask"><h2>'+item.name+'</h2><p>'+item.description+'</p><a href=a href='+item.weblink+' class="info fancybox" rel="group" title="'+item.id+'" ><div class="alt">Click</div></a></div>')
    ...
    $.getJSON('https://api.myjson.com/bins/8fram', function(data){
    ...
    $('.next').on('click', function(event){
        ...
        $.ajax('https://api.myjson.com/bins/8fram', {
